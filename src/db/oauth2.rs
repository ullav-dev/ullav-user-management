...
/// A confidential ("service") client provisioned for the `client_credentials`
/// grant — machine-to-machine callers with no human present, e.g. an AWE
/// automated task. Distinct from `OAuth2Client` (used by the public/DCR
/// authorization_code clients) so that path's row shape is untouched.
#[derive(Debug, Clone)]
...
/// Fetch a confidential service client by id. Returns `AppError::NotFound` if
/// the client doesn't exist or was never provisioned as a service client
/// (`client_secret_hash`/`service_account_user_id` NULL — i.e. it's a public
/// DCR client, not eligible for `client_credentials`).
pub async fn get_service_client(pool: &Pool, client_id: &str) -> Result<ServiceClient, AppError> {
    let conn = pool.get().await?;
    let row = conn
        .query_opt(
            "SELECT client_id, allowed_scopes, client_secret_hash, service_account_user_id
             FROM oauth2_clients
             WHERE client_id = $1 AND client_secret_hash IS NOT NULL
                                   AND service_account_user_id IS NOT NULL",
            &[&client_id],
        )
        .await?
        .ok_or(AppError::NotFound)?;
    Ok(ServiceClient {
        client_id: row.get(0),
        allowed_scopes: row.get(1),
        client_secret_hash: row.get(2),
        service_account_user_id: row.get(3),
    })
}

/// Provision a new confidential service client bound to `service_account_user_id`.
/// `client_secret_hash` is the Argon2 hash of a secret generated by the caller
/// (never stored in plaintext — mirrors `users.password_hash`).
pub async fn create_service_client(
    ...
    conn.execute(
        "INSERT INTO oauth2_clients
            (client_id, client_name, redirect_uris, allowed_scopes,
             service_account_user_id, client_secret_hash)
         VALUES ($1, $2, '{}', $3, $4, $5)",
        &[
            &client_id,
            &client_name,
...
/// List all provisioned service clients (secret hash never included).
pub async fn list_service_clients(pool: &Pool) -> Result<Vec<(String, String, Vec<String>, Uuid)>, AppError> {
    let conn = pool.get().await?;
    let rows = conn
        .query(
            "SELECT client_id, client_name, allowed_scopes, service_account_user_id
             FROM oauth2_clients
...
/// Revoke (delete) a service client. Also invalidates all future
/// `client_credentials` token requests for it; existing access tokens already
/// issued remain valid until they naturally expire (max 1h).
pub async fn delete_service_client(pool: &Pool, client_id: &str) -> Result<(), AppError> {
    let conn = pool.get().await?;
    let n = conn
        .execute(
            "DELETE FROM oauth2_clients WHERE client_id = $1 AND client_secret_hash IS NOT NULL",
            &[&client_id],
        )
        .await?;
    if n == 0 {
